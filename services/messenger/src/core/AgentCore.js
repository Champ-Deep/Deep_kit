...
class AgentCore {
  ...
  detectToolIntent(message) {
    ...
    // --- Send Email ---
    const emailMatch = message.match(/(?:send|compose|write|draft)\s+(?:an?\s+)?email\s+to\s+([^\s]+@[^\s]+)/i);
    if (emailMatch) {
      return { tool: 'send_email', args: { to: emailMatch[1].trim() } };
    }
    if (msg.match(/(?:send|compose|write|draft)\s+(?:an?\s+)?email/i)) {
      return { tool: 'send_email', args: {} };
    }

    // --- Calendar (Google or Local) ---
    if (msg.match(/(?:my\s+)?(?:calendar|schedule|events?)\s*(?:today|for\s+today)/i) ||
        msg.match(/what'?s?\s+(?:on\s+)?(?:my\s+)?(?:calendar|schedule)\s*(?:today)?/i)) {
      return { tool: 'calendar', args: { action: 'today' } };
    }
    if (msg.match(/(?:my\s+)?(?:calendar|schedule|events?)\s+(?:this\s+)?week/i) ||
        msg.match(/what'?s?\s+(?:on\s+)?(?:my\s+)?(?:calendar|schedule)\s+(?:this\s+)?week/i)) {
      return { tool: 'calendar', args: { action: 'week' } };
    }
    const createEventMatch = message.match(/(?:schedule|create|add|book)\s+(?:a\s+)?(?:meeting|event|appointment)\s+(?:called\s+|titled\s+|named\s+)?["']?([^"']+)["']?/i);
    if (createEventMatch) {
      return { tool: 'calendar', args: { action: 'create', title: createEventMatch[1].trim() } };
    }
    ...
    // --- Arsenal Services (Command Center) ---
    // List all services
    if (msg.match(/(?:list|show|get|view)\s*(?:all\s+)?(?:services?|arsenal|containers?)/i) ||
        msg.match(/(?:what'?s?\s+(?:in\s+)?(?:the\s+)?arsenal)/i) ||
        msg === 'services' || msg === 'arsenal') {
      return { tool: 'arsenal_services', args: { action: 'list' } };
    }

    // Service health matrix
    if (msg.match(/(?:service|arsenal)\s*(?:health|status)\s*(?:matrix|overview)?/i) ||
        msg.match(/(?:how\s+(?:are|is)\s+(?:the\s+)?services?)/i)) {
      return { tool: 'arsenal_services', args: { action: 'health' } };
    }

    // Services by zone
    if (msg.match(/(?:show|list|get)\s*(?:services?\s+)?(?:by\s+)?zones?/i) ||
        msg.match(/(?:what'?s?\s+in\s+(?:each\s+)?zone)/i)) {
      return { tool: 'arsenal_services', args: { action: 'zones' } };
    }

    // Service logs
    const serviceLogsMatch = message.match(/(?:show|get|view)\s+logs?\s+(?:for|from)\s+([^\s]+)/i);
    if (serviceLogsMatch) {
      return { tool: 'arsenal_services', args: { action: 'logs', service: serviceLogsMatch[1].trim() } };
    }

    // --- Hardware Status ---
    if (msg.match(/(?:hardware|resource)\s*(?:status|metrics|usage)?/i) ||
        msg.match(/(?:how\s+much\s+(?:ram|memory|cpu))/i) ||
        msg.match(/(?:show\s+me\s+(?:the\s+)?(?:hardware|resources))/i)) {
      return { tool: 'hardware_status', args: { metric: 'all' } };
    }

    // --- Recent Events ---
    if (msg.match(/(?:recent|latest|last)\s*(?:events?|activity|happenings?)/i) ||
        msg.match(/(?:what'?s?\s+(?:been\s+)?happening)/i) ||
        msg.match(/(?:show\s+me\s+(?:the\s+)?(?:events?|activity))/i) ||
        msg === 'events' || msg === 'activity') {
      return { tool: 'recent_events', args: { limit: 20 } };
    }
...
Otherwise, ALWAYS use a tool. Output ONLY the tool JSON block, nothing else.`;
  ...
  inferResponseType(toolName, result) {
    ...
    const toolTypeMap = {
      ...
      'arsenal_services': 'service_list',
      'hardware_status': 'hardware_metrics',
...
Current context: User messaging via DeepKit Messenger (THE PROXY, port 3333).`;
  ...
  getSuggestions(tool, result) {
    const defaults = ['Show arsenal', 'List my tasks', 'Hardware status'];
    if (!tool) return defaults;

    const map = {
      system_info: ['Show arsenal', 'Hardware status', 'Recent events'],
      health_check: ['Show arsenal', 'Hardware status', 'List my tasks'],
      arsenal_services: ['Hardware status', 'Recent events', 'List my tasks'],
      hardware_status: ['Show arsenal', 'Recent events', 'System status'],
      recent_events: ['Show arsenal', 'Hardware status', 'List my tasks'],
      task_tracker: ['List my tasks', 'Create a task', 'Show workflows'],
      notes: ['List my notes', 'Save a note', 'List my tasks'],
      n8n_create: ['Show workflows', 'Create a workflow', 'Show arsenal'],
      n8n_builder: ['Show workflows', 'Trigger workflow', 'Show arsenal'],
      n8n_trigger: ['Show workflows', 'Show arsenal', 'List my tasks'],
      calendar: ['Check my schedule today', 'Create an event', 'Show my tasks'],
      send_email: ['Send another email', 'Check my tasks', 'Show services'],
      content_extract: ['Save as a note', 'Summarize this', 'List my tasks'],
      doc_generate: ['List my tasks', 'Show arsenal', 'Create another doc'],
      docker_discovery: ['Health check', 'Show arsenal', 'Hardware status'],
      web_fetch: ['Show arsenal', 'List my tasks', 'Save a note'],
      file_ops: ['Show arsenal', 'List my tasks', 'Show workflows'],
      process_control: ['Show arsenal', 'Health check', 'List my tasks'],
    };

    // Refine based on action
    if (tool === 'task_tracker') {
      if (result?.action === 'created') return ['List my tasks', 'Create another task', 'Show arsenal'];
      if (result?.action === 'list') return ['Create a task', 'Show workflows', 'Show arsenal'];
    }
    if (tool === 'notes') {
      if (result?.action === 'saved') return ['List my notes', 'Create a task', 'Show arsenal'];
      if (result?.action === 'list') return ['Save a note', 'List my tasks', 'Show arsenal'];
    }
    if (tool === 'arsenal_services') {
      if (result?.action === 'health') return ['Show arsenal', 'Hardware status', 'Recent events'];
      if (result?.action === 'zones') return ['Show arsenal', 'Hardware status', 'Recent events'];
    }
