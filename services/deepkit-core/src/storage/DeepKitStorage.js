...
class DeepKitStorage {
  ...
  async initialize() {
    ...
    try {
      ...
      await client.query(`
        ...
        -- DeepKit settings (key-value store for first-run, preferences, etc.)
        CREATE TABLE IF NOT EXISTS deepkit_settings (
  ...
  async getSetting(key) {
    try {
      const result = await this.pool.query(
        'SELECT value FROM deepkit_settings WHERE key = $1',
        [key]
      );
      return result.rows[0]?.value || null;
    } catch (error) {
      logger.warn(`Failed to get setting ${key}:`, error.message);
      return null;
    }
  }
  ...
  async setSetting(key, value) {
    const sql = `
      INSERT INTO deepkit_settings (key, value, updated_at)
      VALUES ($1, $2, NOW())
    ...
    const result = await this.pool.query(sql, [key, JSON.stringify(value)]);
  ...
  async deleteSetting(key) {
    const result = await this.pool.query(
      'DELETE FROM deepkit_settings WHERE key = $1 RETURNING *',
      [key]
    );
    return result.rows[0] || null;
  }
