...
/**
 * EnvironmentValidator - Validates and generates secrets for DeepKit .env files
...
class EnvironmentValidator {
      ...
      for (const [varName, spec] of Object.entries(config.variables)) {
        ...
        if (spec.type === 'secret' && spec.validation?.entropy) {
          ...
          if (entropy < spec.validation.entropy) {
            this.warnings.push({
              variable: varName,
              warning: `Low entropy (${entropy} < ${spec.validation.entropy}). Consider using a stronger secret.`
            });
          }
        }
      }
  ...
  /**
   * Get all variables that should be auto-generated
   */
  getGeneratableSecrets() {
    const secrets = [];
    for (const [category, config] of Object.entries(this.registry.categories)) {
      for (const [varName, spec] of Object.entries(config.variables)) {
        if (spec.generateOnInstall) {
          secrets.push({ name: varName, spec, category });
        }
      }
    }
    return secrets;
