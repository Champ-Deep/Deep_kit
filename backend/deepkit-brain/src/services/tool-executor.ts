...
import { Tool, ToolMethod } from '../types';
import fs from 'fs';
import path from 'path';

const toolsRegistry = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../../tools-registry/tools.json'), 'utf-8')
);
...
export class ToolExecutor {
  ...
  constructor(timeout: number = 10000, maxRetries: number = 3) {
    this.tools = new Map();
    this.timeout = timeout;
    this.maxRetries = maxRetries;

    // Load tools from registry
    toolsRegistry.tools.forEach((tool: Tool) => {
   ...
   */
  private buildParameterSchema(params: (string | { name: string; optional?: boolean })[]): any {
   ...
   */
  parseToolCall(functionName: string): { toolName: string; methodName: string } | null {
