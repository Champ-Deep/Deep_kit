import { PORT_RANGES, PortSuggestionRequest, PortSuggestionResponse, PortEntry } from '../types';

/**
 * Get port range for a specific type
 */
function getPortRangeForType(type: PortSuggestionRequest['type']): [number, number] {
  switch (type) {
    case 'frontend':
      return [PORT_RANGES.THE_FACE.start, PORT_RANGES.THE_FACE.end];
    case 'backend':
      return [PORT_RANGES.THE_ENGINE.start, PORT_RANGES.THE_ENGINE.end];
    case 'database':
      return [PORT_RANGES.THE_VAULT.start, PORT_RANGES.THE_VAULT.end];
    case 'service':
      return [PORT_RANGES.THE_BRAIN.start, PORT_RANGES.THE_BRAIN.end];
    default:
      return [3000, 9999];
  }
}
 ...
 */
function isPortAvailable(port: number, inUsePorts: Set<number>): boolean {
 ...
 */
function findFirstAvailable(range: [number, number], inUsePorts: Set<number>): number {
 ...
 */
function findCluster(size: number, range: [number, number], inUsePorts: Set<number>): number[] {
 ...
 */
function findAlternatives(recommended: number, range: [number, number], inUsePorts: Set<number>, count: number = 3): number[] {
 ...
 */
export function analyzePort(port: number, currentPorts: PortEntry[]): {
  available: boolean;
  details?: PortEntry;
  zone: string;
  ...
  let zone = 'UNASSIGNED';
  if (port >= 3000 && port <= 3999) zone = 'THE FACE';
  else if (port >= 5000 && port <= 5999) zone = 'THE ENGINE';
  else if (port >= 6000 && port <= 7999) zone = 'THE VAULT';
  else if (port >= 11000 && port <= 11999) zone = 'THE BRAIN';
  else if (port <= 1023) zone = 'SYSTEM (Protected)';
